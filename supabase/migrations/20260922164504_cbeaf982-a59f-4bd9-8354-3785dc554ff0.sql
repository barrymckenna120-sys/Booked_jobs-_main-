...
BEGIN
  ...
  IF NOT p_accepted THEN
    ...
    IF v_quote.boiler_enquiry_id IS NOT NULL THEN
      UPDATE boiler_enquiries
      SET status = 'LOST',
          updated_at = now()
      WHERE id = v_quote.boiler_enquiry_id
        AND NOT EXISTS (
          SELECT 1 FROM quotes
          WHERE boiler_enquiry_id = v_quote.boiler_enquiry_id
            AND status IN ('Draft','Sent','sent','viewed','Pending Approval')
        );
    END IF;
