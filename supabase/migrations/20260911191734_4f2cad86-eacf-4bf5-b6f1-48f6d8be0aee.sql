...
AS $function$
  SELECT json_build_object(
    ...
    'amount_paid', COALESCE((
      ...
      WHERE jp.service_call_id = sc.id
        AND jp.amount > 0
      ORDER BY jp.paid_at DESC, jp.created_at DESC
      LIMIT 1
    ), sc.revenue),
...
GRANT EXECUTE ON FUNCTION public.get_receipt_public(text) TO anon, authenticated, service_role;