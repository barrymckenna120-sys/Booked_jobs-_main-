...
DECLARE
  ...
  v_recipient record;
BEGIN
  SELECT q.status, q.job_id, q.user_id, c.name, q.description, q.customer_id, q.converted_job_id, q.total_amount, q.quote_number, COALESCE(q.deposit, q.deposit_amount, 0)
  INTO v_current_status, v_job_id, v_user_id, v_customer_name, v_description, v_customer_id, v_converted_job_id, v_total_amount, v_quote_number, v_deposit
  FROM quotes q
  ...
  IF p_accepted THEN
    IF v_converted_job_id IS NULL THEN
      ...
      VALUES (v_customer_id, v_user_id, COALESCE(v_orig_job_type, 'Repair'), v_description, v_orig_engineer, v_orig_engineer_id, 'Pending', true, 'Created from accepted quote ' || v_quote_ref, 'Quote', v_total_amount)
    ...
    VALUES (
      ...
      'quote_accepted',
      'Quote Accepted',
      v_quote_ref || ' accepted by ' || COALESCE(v_customer_name, 'Customer') || '. Job created and ready to schedule. Total: ' || chr(8364) || TRIM(TO_CHAR(v_total_amount, 'FM999,999.00')),
      v_new_job_id,
      'office',
