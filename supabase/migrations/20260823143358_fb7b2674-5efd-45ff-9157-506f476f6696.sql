...
ALTER TABLE public.parts_requests
  ...
  CHECK (status = ANY (ARRAY['Open'::text, 'Ordered'::text, 'Ready to Fit'::text, 'Fitted'::text, 'Cancelled'::text]));

ALTER TABLE public.parts_requests
  ADD COLUMN IF NOT EXISTS fitted_at timestamp with time zone,
  ADD COLUMN IF NOT EXISTS fitted_by uuid;

-- 2. Permanent per-part activity trail on the customer record
...
BEGIN
  ...
  ELSE
    CASE NEW.status
      ...
      WHEN 'Fitted' THEN
        v_event_type := 'part_fitted'; v_verb := 'Part fitted'; v_stamp := COALESCE(NEW.fitted_at, now());
      WHEN 'Cancelled' THEN
...
DROP TRIGGER IF EXISTS trg_log_parts_request_activity ON public.parts_requests;
CREATE TRIGGER trg_log_parts_request_activity
AFTER INSERT OR UPDATE ON public.parts_requests
FOR EACH ROW EXECUTE FUNCTION public.log_parts_request_activity();