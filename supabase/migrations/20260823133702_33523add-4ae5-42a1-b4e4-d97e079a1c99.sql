...
BEGIN
  ...
  -- 0) New request logged -> tell the office (BJ-0064). Creation previously
  -- notified nobody: this function only ran on UPDATE.
...
DROP TRIGGER IF EXISTS notify_on_parts_request_change ON public.parts_requests;
CREATE TRIGGER notify_on_parts_request_change
AFTER INSERT OR UPDATE ON public.parts_requests
FOR EACH ROW EXECUTE FUNCTION public.notify_on_parts_request_change();