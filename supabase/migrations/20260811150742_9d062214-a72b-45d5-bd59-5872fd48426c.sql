...
BEGIN
  ...
  IF TG_OP = 'UPDATE'
    ...
    -- Fallback target: engineer assigned to the linked job (engineers.auth_user_id)
    ...
    IF NEW.engineer_id IS NULL AND NEW.assigned_engineer_id IS NULL
       AND NEW.logged_by IS NULL AND v_job_engineer IS NULL THEN
      RETURN NULL;
    END IF;
...
DROP POLICY IF EXISTS "Authenticated users can read boiler_brands" ON public.boiler_brands;
CREATE POLICY "Users read own org boiler_brands or defaults"
ON public.boiler_brands
FOR SELECT
TO authenticated
USING (is_default = true OR organisation_id = public.get_my_org_id());