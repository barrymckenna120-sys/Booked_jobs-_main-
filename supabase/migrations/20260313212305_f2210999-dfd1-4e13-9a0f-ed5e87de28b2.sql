...
BEGIN
  ...
  SELECT q.status, q.job_id, q.user_id, c.name, q.description, q.customer_id, q.converted_job_id
  INTO v_current_status, v_job_id, v_user_id, v_customer_name, v_description, v_customer_id, v_converted_job_id
  FROM quotes q
  ...
  IF p_accepted THEN
    ...
    IF v_converted_job_id IS NULL THEN
      ...
      INSERT INTO service_calls (customer_id, user_id, job_type, job_issue, assigned_engineer, assigned_engineer_id, status, has_quote, notes, source)
      VALUES (v_customer_id, v_user_id, COALESCE(v_orig_job_type, 'Repair'), v_description, v_orig_engineer, v_orig_engineer_id, 'Pending', true, 'Created from quote ' || v_quote_ref, 'Quote')
