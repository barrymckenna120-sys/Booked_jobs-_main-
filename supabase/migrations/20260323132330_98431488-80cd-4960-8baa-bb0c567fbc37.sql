-- FIX 1: Update respond_to_quote to use 'incoming' status for quote-derived jobs
-- FIX 4: Update notification title to 'New Incoming Job'
...
BEGIN
  ...
  IF p_accepted THEN
    IF v_converted_job_id IS NULL THEN
      ...
      VALUES (v_customer_id, v_user_id, COALESCE(v_orig_job_type, 'Repair'), v_description, v_orig_engineer, v_orig_engineer_id, 'incoming', true, 'Created from accepted quote ' || v_quote_ref, 'Quote', v_total_amount)
