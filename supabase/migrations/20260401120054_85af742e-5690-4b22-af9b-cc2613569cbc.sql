...
-- 1. Add quote_id column to service_calls
ALTER TABLE public.service_calls
  ADD COLUMN quote_id uuid REFERENCES public.quotes(id);

-- 2. Update respond_to_quote to store quote_id on the new job
...
BEGIN
  SELECT q.status, q.job_id, q.user_id, c.name, q.description, q.customer_id, q.converted_job_id, q.total_amount, q.quote_number, COALESCE(q.deposit, q.deposit_amount, 0), q.deposit_amount, q.deposit, q.balance_due, q.payment_link, q.job_type
  INTO v_current_status, v_job_id, v_user_id, v_customer_name, v_description, v_customer_id, v_converted_job_id, v_total_amount, v_quote_number, v_deposit, v_deposit_amount, v_effective_deposit, v_balance_due, v_payment_link, v_quote_job_type
  FROM quotes q
  ...
  IF p_accepted THEN
    IF v_converted_job_id IS NULL THEN
      ...
      INSERT INTO service_calls (customer_id, user_id, job_type, job_issue, assigned_engineer, assigned_engineer_id, status, has_quote, notes, source, revenue, deposit_amount, balance_due, deposit_required, deposit_paid, payment_link, quote_id)
      VALUES (v_customer_id, v_user_id, COALESCE(NULLIF(v_quote_job_type, 'other'), v_orig_job_type, 'Repair'), v_description, v_orig_engineer, v_orig_engineer_id, 'incoming', true, 'Created from accepted quote ' || v_quote_ref || E'\n\n' || COALESCE(v_description, ''), 'Quote', v_total_amount, v_effective_deposit, COALESCE(v_balance_due, v_total_amount - v_effective_deposit), (v_effective_deposit > 0), false, v_payment_link, p_quote_id)
