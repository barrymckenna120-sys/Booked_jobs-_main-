...
-- 1. Add balance_due column to service_calls
ALTER TABLE public.service_calls ADD COLUMN IF NOT EXISTS balance_due numeric DEFAULT NULL;

-- 2. Update respond_to_quote RPC to map deposit and balance fields
...
DECLARE
  ...
  v_recipient record;
BEGIN
  SELECT q.status, q.job_id, q.user_id, c.name, q.description, q.customer_id, q.converted_job_id, q.total_amount, q.quote_number, COALESCE(q.deposit, q.deposit_amount, 0), q.deposit_amount, q.balance_due, q.payment_link
  INTO v_current_status, v_job_id, v_user_id, v_customer_name, v_description, v_customer_id, v_converted_job_id, v_total_amount, v_quote_number, v_deposit, v_deposit_amount, v_balance_due, v_payment_link
  FROM quotes q
  ...
  IF p_accepted THEN
    IF v_converted_job_id IS NULL THEN
      ...
      VALUES (v_customer_id, v_user_id, COALESCE(v_orig_job_type, 'Repair'), v_description, v_orig_engineer, v_orig_engineer_id, 'incoming', true, 'Created from accepted quote ' || v_quote_ref, 'Quote', v_total_amount, v_deposit_amount, v_balance_due, (COALESCE(v_deposit, 0) > 0), false, v_payment_link)
    ...
    VALUES (
      ...
      v_new_job_id,
      'office',
      jsonb_build_object('customer_name', v_customer_name, 'quote_ref', v_quote_ref, 'quote_id', p_quote_id, 'total_amount', v_total_amount, 'deposit', v_deposit)
    );
