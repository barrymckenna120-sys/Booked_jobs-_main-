...
- Implement `hasOtherClaimedEvent` with a service-role select on `sumup_webhook_events` filtered by `service_call_id = job` and `checkout_id != current`, limit 1. On query error, fail closed the safe way for money: log the error and treat as "no prior event" only if the error is a not-found; a real query failure returns 500 so SumUp retries rather than risking a double-apply.

`supabase/functions/_shared/sumupWebhook.test.ts`
- Add Deno cases: (a) mis-stamped job (`deposit_paid = true`, no prior event) now processes and patches; (b) prior claimed event under a different checkout id → `duplicate`, no `updateJob` call; (c) layer 1 re-delivery of the same checkout still short-circuits before layer 2.
