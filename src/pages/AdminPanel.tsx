    ...
    (async () => {
      try {
        const { data, error: invokeError } =
          await invokeFunction("list-users", {
            body: { org_id: orgId },
          });
        ...
        if (
          ...
          setError(
            "Failed to load users for this organisation"
          );
          setUsers([]);
          return;
        }
      ...
      >
        ...
        {loading ? (
          ...
          <div className="max-h-72 overflow-y-auto">
            {users.map((u) => {
              const busy =
                unblockingEmail !== null;
              const isThisOne =
                unblockingEmail === u.email;
              const disabled =
                !u.email || busy;
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {blocked ? (
                      <Badge
                        className="text-[10px] bg-red-100 text-red-800 hover:bg-red-100"
                        variant="secondary"
                      >
                        Blocked
                      </Badge>
    ...
    try {
      ...
      const write =
        await supabase
          ...
          .insert({
            organisation_id:
              "62d6c1c3-99cc-47fa-80ce-ea0e36f0d52b",
            payment_type: "pos",
            amount: 1,
            status: "test",
          })
...
export default function AdminPanel() {
  ...
  // Keep both sets of provisioning fields.
  const [
    jobReferencePrefix,
    setJobReferencePrefix,
  ] = useState("");
  const [prefixDirty, setPrefixDirty] =
    useState(false);
  ...
  const [rgiNumber, setRgiNumber] =
    useState("");
  const [wabaId, setWabaId] =
    useState("");
  const [
    apiKeySecret,
    setApiKeySecret,
  ...
  const [
    blockedStatusFetched,
    setBlockedStatusFetched,
  ] = useState(false);
  ...
  const loadTenants = async () => {
    ...
    const res = await fetch(
      `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/list-tenants`,
      {
        headers: {
  ...
  const loadLatestActivity =
    async () => {
      const { data } =
        await supabase
          .from(
            "tenant_activity_log" as any
          )
          .select("*")
          .order("created_at", {
            ascending: false,
        ...
        if (ownerEmail) {
          try {
            ...
            if (
              ...
              toast.error(
                `Blocked, but email failed: ${msg}`
              );
            } else {
              toast.success(
                `${tenant.name} blocked — owner notified`
              );
            }
          } catch (e) {
            toast.error(
              `Blocked, but email failed: ${
                e instanceof Error
                  ? e.message
                  : "unknown"
              }`
          ...
          toast.success(
            `${tenant.name} blocked (no owner email on file)`
          );
        }
  ...
  const handleConfirmArchive =
    async () => {
      if (!archiveModalTenant)
        return;

      if (
        archiveTypedName.trim() !==
        archiveModalTenant.name
      ) {
        return;
      }
  ...
  const handleSubmit = async (
    ...
    if (!orgSlug.trim()) {
      next.org_slug = "Required";
    }

    if (
      !jobReferencePrefix.trim()
    ) {
      next.job_reference_prefix =
        "Required";
    } else if (
      !PREFIX_RE.test(
        jobReferencePrefix
      ...
      next.job_reference_prefix =
        "2–6 characters, uppercase letters or digits only";
    }
    ...
    <div className="container mx-auto max-w-5xl space-y-6 p-6">
      ...
      >
        <TabsList>
          ...
          <TabsTrigger value="unblock-users">
            Unblock Users
          </TabsTrigger>
        ...
        <TabsContent
          ...
          className="space-y-6"
        >
          {/* TEMP DEBUG — remove after RLS verification */}
          ...
          <Card>
            ...
            <CardContent>
              ...
              >
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="job_reference_prefix">
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="api_key_secret">
                    360dialog API Key Secret
                    Name{" "}
                    <span className="text-muted-foreground">
                      (optional)
                    </span>
