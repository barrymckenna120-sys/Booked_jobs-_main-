...
import { AlertTriangle, Check, Clock, Loader2, RefreshCw, ShieldOff } from "lucide-react";
  ...
  return (
    <div
      className={cn(
        ...
        failed ? "border-rose-200 bg-rose-50" : "border-slate-200 bg-white",
        className,
      )}
    >
          ...
          {(attempts ?? []).map((a) => (
            <li key={a.id} className="text-[11px] text-muted-foreground">
              <span className="font-semibold text-foreground">
                #{a.attempt_number} {a.outcome === "sent" ? "Sent" : a.outcome === "opted_out" ? "Opted out" : a.outcome === "failed" ? "Failed" : "Pending"}
              </span>{" "}
