...
const SignatureCanvas = ({
  onConfirm, onBack, title, subtitle,
}: {
  ...
  const move = (e: React.TouchEvent | React.MouseEvent) => {
    ...
    const ctx = canvasRef.current!.getContext("2d")!;
    const p = getPos(e); ctx.lineWidth = 2; ctx.lineCap = "round"; ctx.strokeStyle = "#000";
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex gap-2 px-4 pb-6 pt-2">
        ...
        <Button onClick={() => { if (!hasDrawn.current) return; onConfirm(canvasRef.current!.toDataURL("image/png")); }}
...
const Cert2Flow: React.FC<Cert2FlowProps> = ({ job, customer, engineerName, engineerRgi, engineerPhone, onClose }) => {
  ...
  const generateCertNumber = () => {
    ...
    return `DC-${year}-${String(rand).padStart(4, "0")}`;
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[900] bg-background flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-4 py-4 space-y-4">
            ...
            <div className="bg-card border border-border rounded-2xl p-4 space-y-2">
              ...
              {([
                ...
                ["Engineer Phone", engineerPhone || "—"],
                ["Boiler Make", customer?.boiler_make_model?.split(" ")[0] || job?.boiler_brand || "—"],
                ["Boiler Model", customer?.boiler_make_model || "—"],
            ...
            <div className="space-y-3">
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Premises & Supply</p>
              <div className="space-y-1">
              ...
              {([
                ...
                <label key={key} className="flex items-center gap-3 bg-card border border-border rounded-2xl p-4 cursor-pointer">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => setSafetyChecks((prev) => ({ ...prev, [key]: !!checked }))}
                  />
        ...
        {step === 2 && (
          <div className="px-4 py-4 space-y-4">
            <h2 className="text-lg font-extrabold text-foreground">Gas Readings & Declaration</h2>
