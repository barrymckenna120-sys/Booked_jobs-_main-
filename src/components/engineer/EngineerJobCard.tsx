...
import { Phone, MapPin, StickyNote, Camera, Eye } from "lucide-react";
  ...
  return (
    <>
      <div
        ...
        style={{ borderLeftColor: `hsl(var(--${job.status === "In Progress" ? "warning" : job.status === "Completed" ? "success" : job.status === "Cancelled" ? "destructive" : "primary"}))` }}
      >
        {/* Header */}
        <div className="flex justify-between items-start mb-1">
          <div className="text-base font-extrabold text-foreground flex-1 leading-tight">{customer.name}</div>
        ...
        {/* Last service mini-bar */}
        {customer.last_service_date && (
          <div className="flex items-center gap-1.5 bg-secondary border border-border rounded-lg px-3 py-2 mb-3">
            <span className="text-sm">📅</span>
            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Last: </span>
            <span className="text-[13px] font-bold text-foreground">{customer.last_service_date}</span>
          </div>
        )}
