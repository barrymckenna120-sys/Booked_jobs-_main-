...
const EngineerJobCard = ({ job, customer, onUpdate, isNextJob = false, photos = [] }: EngineerJobCardProps) => {
  ...
  return (
    <>
      ...
      <PartsNeededSheet open={showPartsNeeded} onClose={() => setShowPartsNeeded(false)} onConfirm={(notes, priority) => { const pLabel = priority === "urgent" ? "Urgent" : priority === "low" ? "Low" : "Normal"; onUpdate(job.id, { status: "parts_needed", notes: `Parts Needed [${pLabel}]: ${notes}`, parts_priority: priority, parts_logged_at: new Date().toISOString() }); setShowPartsNeeded(false); toast({ title: "Parts noted — office has been informed" }); }} /> onClose={() => setShowPartsNeeded(false)} onConfirm={(notes, priority) => { const pLabel = priority === "urgent" ? "Urgent" : priority === "low" ? "Low" : "Normal"; onUpdate(job.id, { status: "parts_needed", notes: `Parts Needed [${pLabel}]: ${notes}`, parts_priority: priority, parts_logged_at: new Date().toISOString() }); setShowPartsNeeded(false); toast({ title: "Parts noted — office has been informed" }); }} /> />
      {showTakePayment && (
        <TakePaymentModal
