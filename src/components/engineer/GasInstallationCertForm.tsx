...
const GasInstallationCertForm: React.FC<
  GasInstallationCertFormProps
> = ({ job, customer, engineerInfo, existingCert, onClose, onSaved }) => {
  ...
  const [serialNumber, setSerialNumber] = useState(
    existingCert?.serial_number || ""
  );
  const [gasType, setGasType] = useState(
    existingCert?.gas_type || "nat_gas"
  );
  const [installType, setInstallType] = useState(
    existingCert?.install_type || "new"
  );
  const [gprn, setGprn] = useState(
    existingCert?.gprn || customer?.gprn || ""
  );
  ...
  useEffect(() => {
    if (!gprn && customer?.gprn) {
      setGprn(customer.gprn);
    }
  }, [customer?.gprn]);

  const [premisesEircode, setPremisesEircode] = useState(
    existingCert?.eircode_premises || customer?.eircode || ""
  );
  const [premisesAddress, setPremisesAddress] = useState(
    existingCert?.address_premises || customer?.address || ""
  );
  const [premisesName, setPremisesName] = useState(
    existingCert?.customer_name_premises || customer?.name || ""
  );
  const [premisesTel, setPremisesTel] = useState(
    existingCert?.tel_premises || customer?.phone || ""
  );
  const [ownerEircode, setOwnerEircode] = useState(
    existingCert?.owner_eircode || ""
  );
  const [ownerAddress, setOwnerAddress] = useState(
    existingCert?.owner_address || ""
  );
  const [ownerName, setOwnerName] = useState(
    existingCert?.owner_name || ""
  );
  const [ownerTel, setOwnerTel] = useState(
    existingCert?.owner_tel || ""
  );

  // Appliances
  const [centralHeating, setCentralHeating] = useState(
    existingCert?.central_heating || false
  );
  const [fireOpen, setFireOpen] = useState(
    existingCert?.fire_open || false
  );
  const [fireFlueless, setFireFlueless] = useState(
    existingCert?.fire_flueless || false
  );
  const [fireRSeal, setFireRSeal] = useState(
    existingCert?.fire_r_seal || false
  );
  ...
  const [otherAppliance, setOtherAppliance] = useState(
    existingCert?.other_appliance || ""
  );
  const [pipeworkMaterial, setPipeworkMaterial] = useState(
    existingCert?.pipework_material || "copper"
  );

  // Safety checks
  const [locationCorrect, setLocationCorrect] = useState(
    existingCert?.appliance_location_correct || false
  );
  const [ventilation, setVentilation] = useState(
    existingCert?.adequate_ventilation || false
  );
  const [flueInspected, setFlueInspected] = useState(
    existingCert?.flue_inspected || false
  );
  const [soundnessTest, setSoundnessTest] = useState(
    existingCert?.soundness_test_pass || false
  );

  // Part II
  const [coReading, setCoReading] = useState(
    existingCert?.co_reading || ""
  );
  const [co2Reading, setCo2Reading] = useState(
    existingCert?.co2_reading || ""
  );
  const [coco2Ratio, setCoco2Ratio] = useState(
    existingCert?.coco2_ratio || ""
  );
  const [commissioningDate, setCommissioningDate] = useState<
    Date | undefined
  >(
    existingCert?.commissioning_date
      ? new Date(existingCert.commissioning_date)
      : new Date()
  );
  const [issueDate, setIssueDate] = useState<Date | undefined>(
    existingCert?.issue_date
      ? new Date(existingCert.issue_date)
      : new Date()
  );
  ...
  // Fetch engineer ID
  const [engineerId, setEngineerId] = useState<string | null>(
    existingCert?.engineer_id || null
  );
  ...
  const handleSave = async (
    ...
    if (!engineerId) {
      toast({
        title: "Engineer profile not found",
        variant: "destructive",
      });
      return;
    }
      ...
      if (existingCert?.id) {
        ...
        if (error) {
          ...
          toast({
            title: "No connection",
            description:
              "Certificate saved and will sync automatically when back online",
            variant: "destructive",
          });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background overflow-y-auto">
      ...
      <div className="bg-gradient-to-br from-primary to-primary-dark px-4 pt-12 pb-4 sticky top-0 z-10">
        <button
          onClick={onClose}
          className="flex items-center gap-1.5 text-primary-foreground/80 text-sm font-semibold mb-2"
        >
        ...
        <p className="text-xs text-primary-foreground/60 mt-0.5">
          New gas connection · Declaration of Conformance
        </p>
      </div>
      ...
      <div className="px-4 pb-40 space-y-3">
        ...
        <FieldGroup label="Gas Type">
          <Toggle
            ...
            onChange={setGasType}
            options={[
              { label: "Nat Gas", value: "nat_gas" },
              { label: "LP Gas", value: "lp_gas" },
            ]}
          />
        </FieldGroup>
        ...
        <FieldGroup label="GPRN">
          <Input
            value={gprn}
            onChange={(e) => setGprn(e.target.value)}
            placeholder="GPRN"
            className="h-10"
          />
        </FieldGroup>
        ...
        <FieldGroup label="EIRCODE">
          <Input
            value={premisesEircode}
            onChange={(e) => setPremisesEircode(e.target.value)}
            className="h-10"
          />
        </FieldGroup>
        ...
        <FieldGroup label="Tel">
          <Input
            value={premisesTel}
            onChange={(e) => setPremisesTel(e.target.value)}
            type="tel"
            className="h-10"
          />
        </FieldGroup>
        ...
        <FieldGroup label="EIRCODE">
          <Input
            value={ownerEircode}
            onChange={(e) => setOwnerEircode(e.target.value)}
            className="h-10"
          />
        </FieldGroup>
        ...
        <FieldGroup label="Other Appliance">
          <Input
            ...
            onChange={(e) =>
              setOtherAppliance(e.target.value)
            }
        ...
        <div className="space-y-3">
          {[
            {
              label: "Appliance Location Correct",
              value: locationCorrect,
              set: setLocationCorrect,
            },
            {
              label: "Adequate Permanent Ventilation",
              value: ventilation,
              set: setVentilation,
            },
            {
              label: "Flue Inspected and Adequate",
              value: flueInspected,
              set: setFlueInspected,
            },
            {
              label: "Soundness Test Pass",
              value: soundnessTest,
              set: setSoundnessTest,
            },
          ].map((item) => (
            <div
              key={item.label}
        ...
        <FieldGroup label="CO Reading (ppm)">
          <Input
            value={coReading}
            onChange={(e) => setCoReading(e.target.value)}
            placeholder="e.g. 12"
            className="h-10"
          />
        </FieldGroup>
        ...
        <FieldGroup label="CO₂ Reading (%)">
          <Input
            value={co2Reading}
            onChange={(e) => setCo2Reading(e.target.value)}
            placeholder="e.g. 8.5"
            className="h-10"
          />
        </FieldGroup>
