    ...
    <div className="flex gap-2">
      {options.map((opt) => (
        <button
          ...
          className={`flex-1 h-11 rounded-xl text-sm font-extrabold border-2 transition-colors ${
            value === opt
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 mx-4 mb-2 border-2 border-border rounded-xl bg-white relative overflow-hidden touch-none">
        ...
        <span className="absolute bottom-2 left-6 text-[10px] text-muted-foreground">
          Sign above the line
        </span>
      </div>
      ...
      <div className="flex gap-2 px-4 pb-6 pt-2">
        ...
        <Button
          variant="outline"
          onClick={clear}
          className="h-12 font-bold gap-1"
        >
...
const GasInstallationFlow: React.FC<
  GasInstallationFlowProps
> = ({
  job,
  customer,
  engineerName,
  ...
  const [workCarriedOutOther, setWorkCarriedOutOther] =
    useState("");

  // Step 2 — Pipework & Appliance
  const [pipeworkMaterial, setPipeworkMaterial] =
    useState("Copper");
  ...
  const [traineeNumber, setTraineeNumber] =
    useState("N/A");

  const [hazardIssued, setHazardIssued] =
    useState("No");

  const [hazardNo, setHazardNo] = useState("");
  const [hazardReason, setHazardReason] =
    useState("");

  // Signatures
  const [customerSig, setCustomerSig] =
  ...
  const handleSubmit = async (
    ...
    const prefix =
      ((settingsRow as any)?.cert_prefix || "").trim() ||
      "CERT";
    ...
    const certData = {
      ...
      checks: {
        tightness_test: {
          status: safetyChecks.tightness_test
            ? "pass"
            : "unchecked",
          note: "",
        },
        ...
        purge_complete: {
          status: safetyChecks.purge_complete
            ? "pass"
            : "unchecked",
          note: "",
        },
        ...
        ventilation: {
          status: safetyChecks.ventilation
            ? "pass"
            : "unchecked",
          note: "",
        },
        ...
        flue_inspected: {
          status: safetyChecks.flue_inspected
            ? "pass"
            : "unchecked",
          note: "",
        },
        ...
        appliance_location: {
          status: safetyChecks.appliance_location
            ? "pass"
            : "unchecked",
          note: "",
        },
        ...
        soundness_test: {
          status: safetyChecks.soundness_test
            ? "pass"
            : "unchecked",
          note: "",
        },
    ...
    if (error) {
      console.error(
        "❌ Certificate insert failed, queuing for retry:",
        error.message,
        error
      );
    ...
    console.log(
      "✅ Certificate inserted successfully. newCertId:",
      newCertId,
      "insertedRow:",
      insertedRow
    );
    ...
    if (newCertId) {
      console.log(
        "🚀 Invoking generate-gas-install-pdf with certificate_id:",
        newCertId
      );

      supabase.functions
        .invoke("generate-gas-install-pdf", {
          body: {
        ...
        .then((res) => {
          console.log(
            "📄 generate-gas-install-pdf response:",
            res.data,
            "error:",
            res.error
          );
      ...
      <div className="fixed inset-0 z-[900] bg-background flex flex-col items-center justify-center px-6 text-center gap-4">
        ...
        {whatsappStatus === "failed" && (
          <div className="flex items-center gap-2 text-sm font-bold text-destructive">
            ...
            WhatsApp send failed — check Message Log
          </div>
        )}

        <Button
          onClick={onClose}
          className="mt-4 h-12 px-8 font-bold"
          style={{ backgroundColor: ACCENT }}
        >
          Back to Job
        </Button>
          ...
          <div className="px-4 py-4 space-y-4">
            ...
            <div className="bg-card border border-border rounded-2xl p-4 space-y-2">
              ...
              {(
                ...
                >
                  ...
                  <span className="text-sm font-bold text-foreground text-right max-w-[60%]">
                    {val || "—"}
                  </span>
                </div>
              ))}
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                ...
                <Input
                  value={gprn}
                  onChange={(e) =>
                    setGprn(e.target.value)
                  }
                  placeholder="Enter GPRN"
                  className="h-11"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  placeholder="e.g. Diaphragm, Rotary"
                  className="h-11"
                />
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <Select
                ...
                onValueChange={setWorkCarriedOut}
              >
                ...
                <SelectContent className="z-[950]">
                  <SelectItem value="New Gas Connection">
                    New Gas Connection
                  </SelectItem>
                  <SelectItem value="New Meter Installation">
                    New Meter Installation
                  </SelectItem>
                  <SelectItem value="Meter Replacement">
                    Meter Replacement
                  </SelectItem>
                  <SelectItem value="Gas Pipe Extension">
                    Gas Pipe Extension
                  </SelectItem>
              ...
              {workCarriedOut === "Other" && (
                <Input
                  value={workCarriedOutOther}
                  onChange={(e) =>
                    setWorkCarriedOutOther(
                      e.target.value
                    )
                  }
          ...
          <div className="px-4 py-4 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                ...
                <Input
                  value={pipeworkSize}
                  onChange={(e) =>
                    setPipeworkSize(e.target.value)
                  }
                  placeholder="e.g. 15, 22, 28"
                  className="h-11"
                  inputMode="decimal"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <Input
                  value={pipeworkLength}
                  onChange={(e) =>
                    setPipeworkLength(e.target.value)
                  }
                  placeholder="e.g. 12"
                  className="h-11"
                  inputMode="decimal"
            ...
            <div className="space-y-3">
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setAppliancesConnected(
                    e.target.value
                  )
                }
                placeholder="e.g. Boiler, Hob, Cooker"
            ...
            <div className="space-y-3 pt-2">
              ...
              {(
                [
                  ...
                  keyof typeof safetyChecks,
                  string
                ][]
              ).map(([key, label]) => (
                <label
                  key={key}
                  className="flex items-center gap-3 bg-card border border-border rounded-2xl p-4 cursor-pointer"
                >
                  <Checkbox
            ...
            <div className="grid grid-cols-3 gap-3">
              {(
                [
                  ...
                  keyof typeof readings,
                  string
                ][]
              ).map(([key, label]) => (
                <div
                  key={key}
                  className="bg-card border border-border rounded-2xl p-3 space-y-1"
                >
                  <Label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">
                    {label}
                  </Label>
            ...
            <div className="space-y-3 pt-2">
              ...
              <div className="space-y-1">
                ...
                <Input
                  value={traineeNumber}
                  onChange={(e) =>
                    setTraineeNumber(e.target.value)
                  }
                  className="h-11"
                />
              </div>
              ...
              <ToggleGroup
                label="Notice of Hazard Issued"
                options={["Yes", "No"]}
                value={hazardIssued}
                onChange={setHazardIssued}
              />
                ...
                <>
                  <div className="space-y-1">
                    ...
                    <Input
                      value={hazardNo}
                      onChange={(e) =>
                        setHazardNo(e.target.value)
                      }
                      className="h-11"
                      placeholder="e.g. NZ-2026-0001"
                    />
                  </div>
          ...
          <Button
            onClick={() => {
              if (
                ...
                toast({
                  title:
                    "Please select the type of work carried out",
                  variant: "destructive",
                });
                return;
              }
              ...
              if (
                ...
                toast({
                  title:
                    "Please specify the work carried out",
                  variant: "destructive",
                });
                return;
              }
