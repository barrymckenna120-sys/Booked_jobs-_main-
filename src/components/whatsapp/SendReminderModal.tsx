...
const fillTemplate = (
  ...
  template
    .replace(/{customer_name}/g, customer.name.split(" ")[0])
    .replace(/{date}/g, customer.next_service_due ? new Date(customer.next_service_due).toLocaleDateString("en-IE") : "soon")
    .replace(/{phone}/g, settings?.whatsapp_number || "")
    .replace(/{business_name}/g, settings?.business_name || "Karl's Gas");
...
const SendReminderModal = ({ customer, defaultType = "30 Day Reminder", settings, open, onClose, onSent }: Props) => {
  ...
  useEffect(() => {
    const typeTemplates = templates.filter((t) => t.message_type === messageType);
    const defaultT = typeTemplates.find((t) => t.is_default) || typeTemplates[0];
    if (defaultT) {
      setSelectedTemplateId(defaultT.id);
      setMessageBody(fillTemplate(defaultT.body, customer, settings));
    } else {
      setSelectedTemplateId(null);
      const fallback = settings?.reminder_message_template || DEFAULT_TEMPLATE;
      setMessageBody(fillTemplate(fallback, customer, settings));
    }
  }, [messageType, templates, customer, settings]);

  const handleTemplateSelect = (t: Template) => {
    setSelectedTemplateId(t.id);
    setMessageBody(fillTemplate(t.body, customer, settings));
  };
