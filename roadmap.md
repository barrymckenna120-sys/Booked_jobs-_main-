...
- [ ] K&N Ltd cutover Step 3 — after Step 2, one booking through each form; read back `org_binding_via` (expect `tenant_secret` or `integration_identifier`, not `unbound_claim`) and confirm the jobs land under K&N gas services Ltd.
