...
- [x] BJ-0135 (22/09/26): Boiler Enquiries list rewritten in place (`src/pages/BoilerEnquiries.tsx`) to the QuotesList tab convention — All + all seven statuses with live counts, name/phone search, one batched linked-quotes query (`quotes.boiler_enquiry_id`, most recent per enquiry shown with its status badge), quote column in table + mobile cards. Status dropdown dropped (superseded by tabs); Timeframe/Source/Heating dropdowns kept. Detail page, routes and backend untouched. 814 tests + type check + build pass; tabs/counts/search/empty state/row navigation verified signed-in in the preview.
...
- [x] BJ-0134: live form wording mapped from the real submission logs — mobile ("Moblie No"), name ("Contact details"), boiler type/age/location, radiator count, bathrooms, showers-at-once, water pressure, cylinder, "Priorty", extras multi-select, timeframe, preferred contact and the boiler photos. Tally envelope fields (formName etc.) can no longer be read as answers; file answers keep their URL through flattening.
...
## Cross-tenant activity + login visibility, Phases 1+2 (23/09/26)
- [x] Phase 1: `auth_activity_events` (event, outcome, account email, tenant, IP, browser/OS/device/display mode, failure reason, route) + `auth_activity_access_log`; superadmin-only read via `is_superadmin()`; no anon access.
- [x] Phase 1: `log-auth-event` Edge Function (deployed 23/09/26, `verify_jwt = false` in config.toml) — server-written IP, event allowlist, pre-auth event restriction, per-IP burst limit 40/min, 8KB cap, server-side tenant resolution (`profiles` then `org_for_login_email`).
- [x] Phase 1: app now records sign-in success, failed sign-in (incl. the previously silent network-error path), account lockout, password reset requested, password changed, sign-out. Fire-and-forget — never blocks or breaks auth.
- [x] Phase 1: superadmin "Login Activity" screen (`src/components/admin/LoginActivityTable.tsx`, Admin > User Activity) — tenant picker, success/failure filter, email/IP search, paged newest-first, mobile cards; each open writes an access-log row.
- [x] Phase 2: nightly `purge_activity_logs()` via pg_cron `purge-activity-logs` 03:20 (IP masked at 90 days, events deleted at 12 months, backend logs 30 days, support reports + access log 12 months); retention published in the privacy policy; record of processing in `docs/privacy/logging-and-retention.md`.
